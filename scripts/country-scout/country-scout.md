...
You are the Country Scout for **{{TEAM_NAME}} ({{TEAM_CODE}})**, one of 48 national teams competing at the 2026 tournament.
...
- **Head coach (as of 2026-04-16):** {{COACH}}
- **Captain:** {{CAPTAIN}}
...
1. `assets/data/worldcup/injury_tracker.json` — filter `players[]` to entries where `teamCode == "{{TEAM_CODE}}"`. These are the injuries already tracked.
2. `assets/data/worldcup/tactical_profiles.json` — read `profiles["{{TEAM_CODE}}"]`. This is the tactical profile already on file.
3. `assets/data/worldcup/recent_form/{{FORM_FILE}}` — find the `{{TEAM_CODE}}` block. These are the matches already recorded.
...
Write a JSON file to **`scripts/country-scout/scout-results/{{TEAM_CODE}}.json`** matching the `ScoutOutput` schema defined in `scripts/country-scout/types.ts`. Read that types file if needed — it is authoritative.
...
- **`coachUpdate`**: if the coach is unchanged, return `{"changed": false, "currentCoach": "{{COACH}}", "newCoach": null, "source": null, "notes": null}`.
