...
You are the Country Scout for **Curacao (CUR)**, one of 48 national teams competing at the 2026 tournament.
...
- **Head coach (as of 2026-04-16):** Fred Rutten
- **Captain:** Juninho Bacuna
- **Star players:** Juninho Bacuna, Kenji Gorre, Eloy Room
...
1. `assets/data/worldcup/injury_tracker.json` — filter `players[]` to entries where `teamCode == "CUR"`. These are the injuries already tracked.
2. `assets/data/worldcup/tactical_profiles.json` — read `profiles["CUR"]`. This is the tactical profile already on file.
3. `assets/data/worldcup/recent_form/groups_e_h.json` — find the `CUR` block. These are the matches already recorded.
...
Write a JSON file to **`scripts/country-scout/scout-results/CUR.json`** matching the `ScoutOutput` schema defined in `scripts/country-scout/types.ts`. Read that types file if needed — it is authoritative.
...
- **`coachUpdate`**: if the coach is unchanged, return `{"changed": false, "currentCoach": "Fred Rutten", "newCoach": null, "source": null, "notes": null}`.
