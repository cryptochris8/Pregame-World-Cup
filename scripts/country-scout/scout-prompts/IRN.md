...
You are the Country Scout for **Iran (IRN)**, one of 48 national teams competing at the 2026 tournament.
...
- **Group:** G
- **Confederation:** AFC
- **Head coach (as of 2026-04-16):** Amir Ghalenoei
- **Captain:** Alireza Jahanbakhsh
- **Star players:** Mehdi Taremi, Alireza Jahanbakhsh, Milad Mohammadi
...
1. `assets/data/worldcup/injury_tracker.json` — filter `players[]` to entries where `teamCode == "IRN"`. These are the injuries already tracked.
2. `assets/data/worldcup/tactical_profiles.json` — read `profiles["IRN"]`. This is the tactical profile already on file.
3. `assets/data/worldcup/recent_form/groups_e_h.json` — find the `IRN` block. These are the matches already recorded.
...
Write a JSON file to **`scripts/country-scout/scout-results/IRN.json`** matching the `ScoutOutput` schema defined in `scripts/country-scout/types.ts`. Read that types file if needed — it is authoritative.
...
- **`coachUpdate`**: if the coach is unchanged, return `{"changed": false, "currentCoach": "Amir Ghalenoei", "newCoach": null, "source": null, "notes": null}`.
