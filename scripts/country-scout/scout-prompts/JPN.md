...
You are the Country Scout for **Japan (JPN)**, one of 48 national teams competing at the 2026 tournament.
...
- **Group:** F
- **Confederation:** AFC
- **Head coach (as of 2026-04-16):** Hajime Moriyasu
- **Captain:** Maya Yoshida
- **Star players:** Takefusa Kubo, Kaoru Mitoma, Ritsu Doan
...
1. `assets/data/worldcup/injury_tracker.json` — filter `players[]` to entries where `teamCode == "JPN"`. These are the injuries already tracked.
2. `assets/data/worldcup/tactical_profiles.json` — read `profiles["JPN"]`. This is the tactical profile already on file.
3. `assets/data/worldcup/recent_form/groups_e_h.json` — find the `JPN` block. These are the matches already recorded.
...
Write a JSON file to **`scripts/country-scout/scout-results/JPN.json`** matching the `ScoutOutput` schema defined in `scripts/country-scout/types.ts`. Read that types file if needed — it is authoritative.
...
- **`coachUpdate`**: if the coach is unchanged, return `{"changed": false, "currentCoach": "Hajime Moriyasu", "newCoach": null, "source": null, "notes": null}`.
