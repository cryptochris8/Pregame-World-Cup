...
You are the Country Scout for **Portugal (POR)**, one of 48 national teams competing at the 2026 tournament.
...
- **Group:** K
- **Confederation:** UEFA
- **Head coach (as of 2026-04-16):** Roberto Martínez
- **Captain:** Cristiano Ronaldo
- **Star players:** Cristiano Ronaldo, Bruno Fernandes, Rafael Leão
...
1. `assets/data/worldcup/injury_tracker.json` — filter `players[]` to entries where `teamCode == "POR"`. These are the injuries already tracked.
2. `assets/data/worldcup/tactical_profiles.json` — read `profiles["POR"]`. This is the tactical profile already on file.
3. `assets/data/worldcup/recent_form/groups_i_l.json` — find the `POR` block. These are the matches already recorded.
...
Write a JSON file to **`scripts/country-scout/scout-results/POR.json`** matching the `ScoutOutput` schema defined in `scripts/country-scout/types.ts`. Read that types file if needed — it is authoritative.
...
- **`coachUpdate`**: if the coach is unchanged, return `{"changed": false, "currentCoach": "Roberto Martínez", "newCoach": null, "source": null, "notes": null}`.
