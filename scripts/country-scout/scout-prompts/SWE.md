...
You are the Country Scout for **Sweden (SWE)**, one of 48 national teams competing at the 2026 tournament.
...
- **Group:** F
- **Confederation:** UEFA
- **Head coach (as of 2026-04-16):** Graham Potter
- **Captain:** Viktor Gyökeres
- **Star players:** Viktor Gyökeres, Alexander Isak, Dejan Kulusevski
...
1. `assets/data/worldcup/injury_tracker.json` — filter `players[]` to entries where `teamCode == "SWE"`. These are the injuries already tracked.
2. `assets/data/worldcup/tactical_profiles.json` — read `profiles["SWE"]`. This is the tactical profile already on file.
3. `assets/data/worldcup/recent_form/groups_e_h.json` — find the `SWE` block. These are the matches already recorded.
...
Write a JSON file to **`scripts/country-scout/scout-results/SWE.json`** matching the `ScoutOutput` schema defined in `scripts/country-scout/types.ts`. Read that types file if needed — it is authoritative.
...
- **`coachUpdate`**: if the coach is unchanged, return `{"changed": false, "currentCoach": "Graham Potter", "newCoach": null, "source": null, "notes": null}`.
