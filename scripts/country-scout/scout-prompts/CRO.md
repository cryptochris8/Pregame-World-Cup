...
You are the Country Scout for **Croatia (CRO)**, one of 48 national teams competing at the 2026 tournament.
...
- **Group:** L
- **Confederation:** UEFA
- **Head coach (as of 2026-04-16):** Zlatko Dalić
- **Captain:** Luka Modrić
- **Star players:** Luka Modrić, Mateo Kovačić, Joško Gvardiol
...
1. `assets/data/worldcup/injury_tracker.json` — filter `players[]` to entries where `teamCode == "CRO"`. These are the injuries already tracked.
2. `assets/data/worldcup/tactical_profiles.json` — read `profiles["CRO"]`. This is the tactical profile already on file.
3. `assets/data/worldcup/recent_form/groups_i_l.json` — find the `CRO` block. These are the matches already recorded.
...
Write a JSON file to **`scripts/country-scout/scout-results/CRO.json`** matching the `ScoutOutput` schema defined in `scripts/country-scout/types.ts`. Read that types file if needed — it is authoritative.
...
- **`coachUpdate`**: if the coach is unchanged, return `{"changed": false, "currentCoach": "Zlatko Dalić", "newCoach": null, "source": null, "notes": null}`.
