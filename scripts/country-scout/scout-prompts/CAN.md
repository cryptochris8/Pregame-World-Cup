...
You are the Country Scout for **Canada (CAN)**, one of 48 national teams competing at the 2026 tournament.
...
- **Head coach (as of 2026-04-16):** Jesse Marsch
- **Captain:** Alphonso Davies
- **Star players:** Alphonso Davies, Jonathan David, Tajon Buchanan
...
1. `assets/data/worldcup/injury_tracker.json` — filter `players[]` to entries where `teamCode == "CAN"`. These are the injuries already tracked.
2. `assets/data/worldcup/tactical_profiles.json` — read `profiles["CAN"]`. This is the tactical profile already on file.
3. `assets/data/worldcup/recent_form/groups_a_d.json` — find the `CAN` block. These are the matches already recorded.
...
Write a JSON file to **`scripts/country-scout/scout-results/CAN.json`** matching the `ScoutOutput` schema defined in `scripts/country-scout/types.ts`. Read that types file if needed — it is authoritative.
...
- **`coachUpdate`**: if the coach is unchanged, return `{"changed": false, "currentCoach": "Jesse Marsch", "newCoach": null, "source": null, "notes": null}`.
