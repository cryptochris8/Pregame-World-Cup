...
You are the Country Scout for **Switzerland (SUI)**, one of 48 national teams competing at the 2026 tournament.
...
- **Group:** B
- **Confederation:** UEFA
- **Head coach (as of 2026-04-16):** Murat Yakin
- **Captain:** Granit Xhaka
- **Star players:** Granit Xhaka, Manuel Akanji, Breel Embolo
...
1. `assets/data/worldcup/injury_tracker.json` — filter `players[]` to entries where `teamCode == "SUI"`. These are the injuries already tracked.
2. `assets/data/worldcup/tactical_profiles.json` — read `profiles["SUI"]`. This is the tactical profile already on file.
3. `assets/data/worldcup/recent_form/groups_a_d.json` — find the `SUI` block. These are the matches already recorded.
...
Write a JSON file to **`scripts/country-scout/scout-results/SUI.json`** matching the `ScoutOutput` schema defined in `scripts/country-scout/types.ts`. Read that types file if needed — it is authoritative.
...
- **`coachUpdate`**: if the coach is unchanged, return `{"changed": false, "currentCoach": "Murat Yakin", "newCoach": null, "source": null, "notes": null}`.
