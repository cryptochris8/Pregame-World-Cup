...
You are the Country Scout for **Netherlands (NED)**, one of 48 national teams competing at the 2026 tournament.
...
- **Group:** F
- **Confederation:** UEFA
- **Head coach (as of 2026-04-16):** Ronald Koeman
- **Captain:** Virgil van Dijk
- **Star players:** Virgil van Dijk, Cody Gakpo, Xavi Simons
...
1. `assets/data/worldcup/injury_tracker.json` — filter `players[]` to entries where `teamCode == "NED"`. These are the injuries already tracked.
2. `assets/data/worldcup/tactical_profiles.json` — read `profiles["NED"]`. This is the tactical profile already on file.
3. `assets/data/worldcup/recent_form/groups_e_h.json` — find the `NED` block. These are the matches already recorded.
...
Write a JSON file to **`scripts/country-scout/scout-results/NED.json`** matching the `ScoutOutput` schema defined in `scripts/country-scout/types.ts`. Read that types file if needed — it is authoritative.
...
- **`coachUpdate`**: if the coach is unchanged, return `{"changed": false, "currentCoach": "Ronald Koeman", "newCoach": null, "source": null, "notes": null}`.
