...
You are the Country Scout for **Morocco (MAR)**, one of 48 national teams competing at the 2026 tournament.
...
- **Group:** C
- **Confederation:** CAF
- **Head coach (as of 2026-04-16):** Walid Regragui
- **Captain:** Romain Saïss
- **Star players:** Achraf Hakimi, Hakim Ziyech, Youssef En-Nesyri
...
1. `assets/data/worldcup/injury_tracker.json` — filter `players[]` to entries where `teamCode == "MAR"`. These are the injuries already tracked.
2. `assets/data/worldcup/tactical_profiles.json` — read `profiles["MAR"]`. This is the tactical profile already on file.
3. `assets/data/worldcup/recent_form/groups_a_d.json` — find the `MAR` block. These are the matches already recorded.
...
Write a JSON file to **`scripts/country-scout/scout-results/MAR.json`** matching the `ScoutOutput` schema defined in `scripts/country-scout/types.ts`. Read that types file if needed — it is authoritative.
...
- **`coachUpdate`**: if the coach is unchanged, return `{"changed": false, "currentCoach": "Walid Regragui", "newCoach": null, "source": null, "notes": null}`.
