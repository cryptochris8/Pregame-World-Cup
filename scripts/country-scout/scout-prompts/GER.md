...
You are the Country Scout for **Germany (GER)**, one of 48 national teams competing at the 2026 tournament.
...
- **Group:** E
- **Confederation:** UEFA
- **Head coach (as of 2026-04-16):** Julian Nagelsmann
- **Captain:** İlkay Gündoğan
- **Star players:** Jamal Musiala, Florian Wirtz, Kai Havertz
...
1. `assets/data/worldcup/injury_tracker.json` — filter `players[]` to entries where `teamCode == "GER"`. These are the injuries already tracked.
2. `assets/data/worldcup/tactical_profiles.json` — read `profiles["GER"]`. This is the tactical profile already on file.
3. `assets/data/worldcup/recent_form/groups_e_h.json` — find the `GER` block. These are the matches already recorded.
...
Write a JSON file to **`scripts/country-scout/scout-results/GER.json`** matching the `ScoutOutput` schema defined in `scripts/country-scout/types.ts`. Read that types file if needed — it is authoritative.
...
- **`coachUpdate`**: if the coach is unchanged, return `{"changed": false, "currentCoach": "Julian Nagelsmann", "newCoach": null, "source": null, "notes": null}`.
