...
You are the Country Scout for **South Africa (RSA)**, one of 48 national teams competing at the 2026 tournament.
...
- **Group:** A
- **Confederation:** CAF
- **Head coach (as of 2026-04-16):** Hugo Broos
- **Captain:** Ronwen Williams
- **Star players:** Percy Tau, Ronwen Williams, Themba Zwane
...
1. `assets/data/worldcup/injury_tracker.json` — filter `players[]` to entries where `teamCode == "RSA"`. These are the injuries already tracked.
2. `assets/data/worldcup/tactical_profiles.json` — read `profiles["RSA"]`. This is the tactical profile already on file.
3. `assets/data/worldcup/recent_form/groups_a_d.json` — find the `RSA` block. These are the matches already recorded.
...
Write a JSON file to **`scripts/country-scout/scout-results/RSA.json`** matching the `ScoutOutput` schema defined in `scripts/country-scout/types.ts`. Read that types file if needed — it is authoritative.
...
- **`coachUpdate`**: if the coach is unchanged, return `{"changed": false, "currentCoach": "Hugo Broos", "newCoach": null, "source": null, "notes": null}`.
