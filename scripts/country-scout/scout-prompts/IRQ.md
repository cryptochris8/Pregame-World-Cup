...
You are the Country Scout for **Iraq (IRQ)**, one of 48 national teams competing at the 2026 tournament.
...
- **Group:** I
- **Confederation:** AFC
- **Head coach (as of 2026-04-16):** Graham Arnold
- **Captain:** Mohanad Ali
- **Star players:** Mohanad Ali, Ibrahim Bayesh, Aymen Hussein
...
1. `assets/data/worldcup/injury_tracker.json` — filter `players[]` to entries where `teamCode == "IRQ"`. These are the injuries already tracked.
2. `assets/data/worldcup/tactical_profiles.json` — read `profiles["IRQ"]`. This is the tactical profile already on file.
3. `assets/data/worldcup/recent_form/groups_i_l.json` — find the `IRQ` block. These are the matches already recorded.
...
Write a JSON file to **`scripts/country-scout/scout-results/IRQ.json`** matching the `ScoutOutput` schema defined in `scripts/country-scout/types.ts`. Read that types file if needed — it is authoritative.
...
- **`coachUpdate`**: if the coach is unchanged, return `{"changed": false, "currentCoach": "Graham Arnold", "newCoach": null, "source": null, "notes": null}`.
