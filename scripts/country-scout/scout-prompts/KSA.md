...
You are the Country Scout for **Saudi Arabia (KSA)**, one of 48 national teams competing at the 2026 tournament.
...
- **Group:** H
- **Confederation:** AFC
- **Head coach (as of 2026-04-16):** Herve Renard
- **Captain:** Salman Al-Faraj
- **Star players:** Salem Al-Dawsari, Saud Abdulhamid, Firas Al-Buraikan
...
1. `assets/data/worldcup/injury_tracker.json` — filter `players[]` to entries where `teamCode == "KSA"`. These are the injuries already tracked.
2. `assets/data/worldcup/tactical_profiles.json` — read `profiles["KSA"]`. This is the tactical profile already on file.
3. `assets/data/worldcup/recent_form/groups_e_h.json` — find the `KSA` block. These are the matches already recorded.
...
Write a JSON file to **`scripts/country-scout/scout-results/KSA.json`** matching the `ScoutOutput` schema defined in `scripts/country-scout/types.ts`. Read that types file if needed — it is authoritative.
...
- **`coachUpdate`**: if the coach is unchanged, return `{"changed": false, "currentCoach": "Herve Renard", "newCoach": null, "source": null, "notes": null}`.
