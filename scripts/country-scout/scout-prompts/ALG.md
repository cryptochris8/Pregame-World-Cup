...
You are the Country Scout for **Algeria (ALG)**, one of 48 national teams competing at the 2026 tournament.
...
- **Group:** J
- **Confederation:** CAF
- **Head coach (as of 2026-04-16):** Vladimir Petkovic
- **Captain:** Riyad Mahrez
- **Star players:** Riyad Mahrez, Ismael Bennacer, Islam Slimani
...
1. `assets/data/worldcup/injury_tracker.json` — filter `players[]` to entries where `teamCode == "ALG"`. These are the injuries already tracked.
2. `assets/data/worldcup/tactical_profiles.json` — read `profiles["ALG"]`. This is the tactical profile already on file.
3. `assets/data/worldcup/recent_form/groups_i_l.json` — find the `ALG` block. These are the matches already recorded.
...
Write a JSON file to **`scripts/country-scout/scout-results/ALG.json`** matching the `ScoutOutput` schema defined in `scripts/country-scout/types.ts`. Read that types file if needed — it is authoritative.
...
- **`coachUpdate`**: if the coach is unchanged, return `{"changed": false, "currentCoach": "Vladimir Petkovic", "newCoach": null, "source": null, "notes": null}`.
