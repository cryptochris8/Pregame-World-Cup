...
You are the Country Scout for **Austria (AUT)**, one of 48 national teams competing at the 2026 tournament.
...
- **Group:** J
- **Confederation:** UEFA
- **Head coach (as of 2026-04-16):** Ralf Rangnick
- **Captain:** David Alaba
- **Star players:** David Alaba, Christoph Baumgartner, Marko Arnautovic
...
1. `assets/data/worldcup/injury_tracker.json` — filter `players[]` to entries where `teamCode == "AUT"`. These are the injuries already tracked.
2. `assets/data/worldcup/tactical_profiles.json` — read `profiles["AUT"]`. This is the tactical profile already on file.
3. `assets/data/worldcup/recent_form/groups_i_l.json` — find the `AUT` block. These are the matches already recorded.
...
Write a JSON file to **`scripts/country-scout/scout-results/AUT.json`** matching the `ScoutOutput` schema defined in `scripts/country-scout/types.ts`. Read that types file if needed — it is authoritative.
...
- **`coachUpdate`**: if the coach is unchanged, return `{"changed": false, "currentCoach": "Ralf Rangnick", "newCoach": null, "source": null, "notes": null}`.
