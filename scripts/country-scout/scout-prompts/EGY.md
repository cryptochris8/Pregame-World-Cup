...
You are the Country Scout for **Egypt (EGY)**, one of 48 national teams competing at the 2026 tournament.
...
- **Group:** G
- **Confederation:** CAF
- **Head coach (as of 2026-04-16):** Hossam Hassan
- **Captain:** Mohamed Salah
- **Star players:** Mohamed Salah, Omar Marmoush, Mohamed Elneny
...
1. `assets/data/worldcup/injury_tracker.json` — filter `players[]` to entries where `teamCode == "EGY"`. These are the injuries already tracked.
2. `assets/data/worldcup/tactical_profiles.json` — read `profiles["EGY"]`. This is the tactical profile already on file.
3. `assets/data/worldcup/recent_form/groups_e_h.json` — find the `EGY` block. These are the matches already recorded.
...
Write a JSON file to **`scripts/country-scout/scout-results/EGY.json`** matching the `ScoutOutput` schema defined in `scripts/country-scout/types.ts`. Read that types file if needed — it is authoritative.
...
- **`coachUpdate`**: if the coach is unchanged, return `{"changed": false, "currentCoach": "Hossam Hassan", "newCoach": null, "source": null, "notes": null}`.
