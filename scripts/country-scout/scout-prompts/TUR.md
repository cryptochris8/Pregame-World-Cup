...
You are the Country Scout for **Turkey (TUR)**, one of 48 national teams competing at the 2026 tournament.
...
- **Group:** D
- **Confederation:** UEFA
- **Head coach (as of 2026-04-16):** Vincenzo Montella
- **Captain:** Hakan Çalhanoğlu
- **Star players:** Hakan Çalhanoğlu, Arda Güler, Kenan Yıldız
...
1. `assets/data/worldcup/injury_tracker.json` — filter `players[]` to entries where `teamCode == "TUR"`. These are the injuries already tracked.
2. `assets/data/worldcup/tactical_profiles.json` — read `profiles["TUR"]`. This is the tactical profile already on file.
3. `assets/data/worldcup/recent_form/groups_a_d.json` — find the `TUR` block. These are the matches already recorded.
...
Write a JSON file to **`scripts/country-scout/scout-results/TUR.json`** matching the `ScoutOutput` schema defined in `scripts/country-scout/types.ts`. Read that types file if needed — it is authoritative.
...
- **`coachUpdate`**: if the coach is unchanged, return `{"changed": false, "currentCoach": "Vincenzo Montella", "newCoach": null, "source": null, "notes": null}`.
