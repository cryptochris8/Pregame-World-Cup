...
void main() {
  ...
  const testUserId = 'watch-parties-admin-uid';
  ...
  group('AdminWatchPartiesScreen - Disposal safety', () {
    testWidgets('can be disposed during async load without crashing',
        (tester) async {
      await seedAdmin();

      await tester.pumpWidget(buildTestWidget());
      // Widget is now loading (awaiting getWatchParties)
    ...
    testWidgets('renders and settles without error', (tester) async {
      ...
      expect(find.byType(AdminWatchPartiesScreen), findsOneWidget);
    });
  });
}
